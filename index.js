...
module.exports = function (dir, randomAccessInterface) {
  return {
    ...
    content: function (name, opts, archive) {
      ...
      if (name === 'data') return createStorage(archive, dir, randomAccessInterface)
      return randomAccessInterface(path.join(dir, '.dat/content.' + name))
    }
  }
}

function createStorage (archive, dir, randomAccessInterface) {
  ...
  function onappending (name, opts) {
    ...
    head = {
      ...
      storage: randomAccessInterface(name + v)
    }
    ...
    archive.ready(function (err) {
      ...
      find(archive.metadata, offset, function (err, node, st, index) {
        ...
        cb(null, {
          ...
          storage: randomAccessInterface(node.name + v)
        })
      })
    })
  }
